...
test("prefers a higher-ranked model", () => {
  ...
  assert.equal(result.selected.canonical, canonicalModel(opus1m));
  assert.equal(result.escalated, true);
  assert.equal(result.intelligencePreference, "higher");
...
test("uses the largest-context route when equal candidates share a model", () => {
  ...
  assert.equal(result.intelligencePreference, "higher");
  assert.match(result.notice ?? "", /Higher intelligence was not available/);
});
...
test("treats dated Claude 4 IDs as version 4.0 rather than huge minor versions", () => {
  const datedOpus4 = model("anthropic", "claude-opus-4-20250514", { contextWindow: 200_000 });
  const result = select(sol, [sol, datedOpus4, opus1m]);
  assert.equal(result.selected.canonical, canonicalModel(opus1m));
});

test("falls back to an equal-ranked peer before the current model", () => {
  const otherSolRoute = model("openai-direct", "openai/gpt-5.6-sol", { contextWindow: 128_000 });
  const result = select(sol, [sol, otherSolRoute]);
  assert.equal(result.selected.canonical, canonicalModel(otherSolRoute));
...
test("reuses the current model when it is the only eligible peer", () => {
  ...
  assert.equal(result.selected.canonical, canonicalModel(sol));
...
test("same intelligence preference restricts to equal-ranked models", () => {
  ...
  assert.equal(result.selected.canonical, canonicalModel(terra));
  assert.equal(result.escalated, false);
  assert.equal(result.intelligencePreference, "same");
...
test("excludes unknown candidates and reports them", () => {
  const unknown = model("custom", "acme/reviewer-ultra");
  const result = select(sol, [sol, unknown]);
  assert.deepEqual(result.excludedUnknown, [canonicalModel(unknown)]);
  assert.equal(result.selected.canonical, canonicalModel(sol));
});
...
test("does not infer tiers from unrelated display-name branding", () => {
  ...
  assert.deepEqual(result.excludedUnknown, [canonicalModel(nanoBanana)]);
});
